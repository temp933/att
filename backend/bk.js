...
//       late_hours: lateHoursDecimal, // 👉 1.3
//       late_text: lateHoursFormatted, // 👉 "1hr 3min"
...
// //     // Close visits to other sites (any session) that are still open
// //     await dbRun(
// //       `UPDATE employee_site_attendance
// //        SET out_time = NOW(), updated_at = NOW(), status = 'completed'
// //        WHERE employee_id = ? AND site_id != ? AND work_date = CURDATE()
...
// //     // Find the most recent row for this employee + site today (any session)
// //     const existing = await dbGet(
...
// //     // Different session = manual START pressed → always new row
// //     const sameSession =
// //       !session_id || !existing.session_id || existing.session_id === session_id;

// //     // Same session + returned within 15 min = reopen (GPS drift/brief exit)
// //     if (
// //       sameSession &&
// //       existing.minutes_since_out !== null &&
// //       existing.minutes_since_out < 15
...
// //     // New session or >15 min away → always new row
// //     const r = await dbRun(
// //       `INSERT INTO employee_site_attendance
// //          (employee_id, site_id, session_id, in_time, work_date, status, updated_at)
// //        VALUES (?, ?, ?, NOW(), CURDATE(), 'active', NOW())`,
...
//           if (!existing) {
//             // No row for this session — always new
//             await dbRun(
//               `INSERT INTO employee_site_attendance
//          (employee_id, site_id, session_id, in_time, work_date, status, updated_at)
//        VALUES (?, ?, ?, ?, ?, 'active', NOW())`,
...
//     // 2. All sessions for that date
//     const sessions = await dbAll(
//       `SELECT ts.id AS session_id, ts.employee_id, ts.session_number,
//           ts.started_at, ts.ended_at, ts.end_reason,
...
//     // 3. All site visits for that date
//     const visits = await dbAll(
//       `SELECT a.id AS visit_id, a.employee_id, a.session_id,
//           s.site_name,
...
//     const activeSession = await dbGet(
//       `SELECT ts.id AS session_id, ts.session_number, ts.started_at, ts.is_late
//        FROM tracking_sessions ts
...
//     // Fetch the first session of today to know if late
//     const firstSession = await dbGet(
//       `SELECT started_at, is_late, late_minutes FROM tracking_sessions
//    WHERE employee_id = ? AND work_date = CURDATE()
...
//       `SELECT esa.id, esa.in_time, esa.out_time, esa.status,
...
//     const firstCheckInTime = firstSession?.started_at || null;
...
//       firstCheckInTime, // actual timestamp of first check-in
//       lateMessage: isLate ? "Late entry" : null,
...
//       lateMinutes,
//       lateHours: lateHoursDecimal, // 👉 1.3
//       lateText, // 👉 "1hr 3min"
...
function calcLateEntry(startedAt) {
  ...
  const cutoff = new Date(d);
  cutoff.setHours(10, 0, 0, 0); // ← changed from 9:30 to 10:00

  // Half Day Logic (after 1 PM)
  if (d.getHours() >= 13) {
    // ← changed from 12 to 13 (1 PM makes more sense for 10-6 shift)
    return {
      isLate: true,
      lateMinutes: 0,
      lateHoursDecimal: 0,
      lateHoursFormatted: "Half Day",
    };
  }
...
app.get("/attendance/today-summary/:empId", async (req, res) => {
  try {
    ...
    // Fetch the first session of today to read stored late values from DB
    const firstSession = await dbGet(
      `SELECT started_at, is_late, late_minutes, late_hours_decimal, late_hours_text
       FROM tracking_sessions
       WHERE employee_id = ? AND work_date = CURDATE()
       ORDER BY id ASC LIMIT 1`,
      [empId],
