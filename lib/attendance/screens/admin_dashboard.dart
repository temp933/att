...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // ── THE FIX ────────────────────────────────────────────────────────────────
  // Root cause: `isExpanded` (a bool) flips to true the instant the mouse
  // enters — BEFORE AnimatedContainer has moved at all.  The wide Row layout
  // (icon 20 + gap 10 + text + dot 10 = ~50 px min) was being rendered inside
  // a container still only ~39 px wide → 1 px overflow every time.
  //
  // Solution: wrap the sidebar contents in LayoutBuilder and switch layouts
  // based on the *actual rendered width*, not the boolean.  ClipRect prevents
  // any residual paint overflow during the slide animation.
