...
class _LeaveScreenState extends State<LeaveScreen>
  ...
  int get _rejectedDays => leaves
      .where(
        (e) =>
            e['status'] == 'Rejected_By_TL' || e['status'] == 'Rejected_By_HR',
      )
      .fold<int>(0, (s, e) => s + (e['number_of_days'] as int));
  ...
  // Tablet/Desktop: 2-column leave card grid
  Widget _buildLeaveGrid(Responsive r) {
    return LayoutBuilder(
      builder: (ctx, constraints) {
        final cols = r.isDesktop ? 2 : 2;
        final gap = 12.0;
  ...
  // ─── Sliver AppBar ────────────────────────────────────────────────────────────
  Widget _buildSliverAppBar(Responsive r) => SliverAppBar(
  ...
  Widget _buildFAB(Responsive r) {
    if (r.isDesktop) {
      // Desktop: show FAB as a full button fixed bottom-right
...
class _LeaveCardState extends State<_LeaveCard> {
    ...
    return Padding(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: _expanded ? statusColor.withOpacity(0.3) : _border,
  ...
  Widget _buildDetails(Map<String, dynamic> leave, Color statusColor) {
    final hasTL = leave['recommended_by'] != null;
    final hasHR = leave['approved_by'] != null;
          ...
          child: Column(
            ...
            children: [
              _infoBlock(
                ...
                title: 'Duration',
                content:
                    '${leave['number_of_days']} day${leave['number_of_days'] == 1 ? '' : 's'}  ·  ${_fmtDate(leave['leave_start_date'])}  →  ${_fmtDate(leave['leave_end_date'])}',
              ),
              ...
              if (hasTL) ...[
                _actionBlock(
                  ...
                  person: widget.resolvePerson(leave['recommended_by']),
                  timestamp: _fmtDateTime(leave['recommended_at']),
                  statusLabel: leave['status'] == 'Rejected_By_TL'
                      ? 'Rejected'
                      : 'Recommended',
                  statusColor: leave['status'] == 'Rejected_By_TL'
                      ? _red
                      : _accent,
                  remark: leave['status'] == 'Rejected_By_TL' && hasRejection
                      ? leave['rejection_reason']
                      : null,
                ),
                const SizedBox(height: 10),
              ] else if (leave['status'] == 'Rejected_By_TL') ...[
                _actionBlock(
                  icon: Icons.supervisor_account_rounded,
                  title: 'Team Lead Review',
                  person: 'Team Lead',
                  ...
                  statusColor: _red,
                  remark: hasRejection ? leave['rejection_reason'] : null,
                ),
                const SizedBox(height: 10),
              ],
              if (hasHR) ...[
                _actionBlock(
                  ...
                  title: 'HR / Admin Action',
                  person: widget.resolvePerson(leave['approved_by']),
                  timestamp: null,
                  statusLabel: leave['status'] == 'Approved'
                      ? 'Approved'
                      : 'Rejected',
                  statusColor: leave['status'] == 'Approved' ? _accent : _red,
                  remark: leave['status'] == 'Rejected_By_HR' && hasRejection
                      ? leave['rejection_reason']
                      : null,
                ),
                const SizedBox(height: 10),
      ...
      children: [
        ...
        if (remark != null && remark.isNotEmpty) ...[
          ...
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Icon(Icons.comment_outlined, size: 13, color: _textLight),
              const SizedBox(width: 4),
              Expanded(
                child: Text(
                  remark,
...
class _ApplyLeaveSheetState extends State<_ApplyLeaveSheet> {
  ...
  Widget build(BuildContext context) {
    final r = Responsive.of(context);
    // On tablet/desktop, show as a centered dialog-style sheet
